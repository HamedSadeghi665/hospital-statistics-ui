...
const MainDashboard = () => {
  const [accessiblePageCategories, setAccessiblePageCategories] = useState(
    []
  );

  useMount(async () => {
    const accessibleModuleCategories =
      await pagesService.accessiblePageCategories();

    setAccessiblePageCategories(accessiblePageCategories);
  });

  return (
    <Row gutter={[10, 16]}>
      {accessiblePageCategories.map((category) => (
